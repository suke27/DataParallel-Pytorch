...
def train(trainloader, model, criterion, optimizer, device_ids):
    model.train()
    ...
    for i, (inputs, labels) in enumerate(trainloader):
        ...
        #outputs = nn.parallel.data_parallel(model, inputs_var)#, device_ids=device_ids)
...
def create_device_ids(gpu):
    gpus = gpu.split(',')
    device_ids = [int(gpu) for gpu in gpus]
    return device_ids

def main():
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    device_ids = create_device_ids(args.gpu)
    nclasses = 10
    epochs = args.epochs
    learning_rate = 0.1
    ...
    model = model.cuda()
    model = nn.parallel.DataParallel(model)#, device_ids=device_ids, output_device=device_ids[0])
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_acc = train(trainloader, model, criterion, optimizer, device_ids)
